...
{
    public sealed class DoAfterQTE
    {
        public Task<DoAfterQTEStatus> AsTask { get; }

        private TaskCompletionSource<DoAfterQTEStatus> Tcs { get; }

        public readonly DoAfterQTEEventArgs EventArgs;
...
#pragma warning disable RA0004
        public DoAfterQTEStatus Status => AsTask.IsCompletedSuccessfully ? AsTask.Result : DoAfterQTEStatus.Running;
#pragma warning restore RA0004
        ...
        public DoAfterQTE(DoAfterQTEEventArgs eventArgs, IEntityManager entityManager)
        {
            EventArgs = eventArgs;
            ...
            Tcs = new TaskCompletionSource<DoAfterQTEStatus>();
            AsTask = Tcs.Task;
        }

        public void Cancel()
        {
            if (Status == DoAfterQTEStatus.Running)
                Tcs.SetResult(DoAfterQTEStatus.Cancelled);
        }
        ...
        public void Finish()
        {
            if (Status == DoAfterQTEStatus.Running)
                if (!TryPostCheck())
                    ...
                    Tcs.SetResult(DoAfterQTEStatus.Finished);
        }
